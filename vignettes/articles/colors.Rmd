...
Let's start by mapping GDP values directly to the `"Blues"` palette from [Color Brewer 2](https://colorbrewer2.org/). We'll use `colorNumeric()` to create a mapping function. The `"Blues"` palette only contains nine colors, but `colorNumeric()` interpolates these colors so we get continuous output.
