...
- `sf` `data.frame` objects (as well as `POINT`, `LINE`, `POLYGON`, etc. objects) from the `{sf}` package
- The data frame from returned from `maps::map()`

The `data` argument is used to derive spatial data for functions that need it; for example, if `data` is a `sf` `data.frame` object, then calling `addPolygons()` on that map widget will know to add the polygons from that `sf` `data.frame`.
...
Below are examples of using `{sf}` and `{maps}`, respectively:

```{r message=FALSE}
library(sf)
Sr1 <- st_polygon(list(cbind(c(2, 4, 4, 1, 2), c(2, 3, 5, 4, 2))))
Sr2 <- st_polygon(list(cbind(c(5, 4, 2, 5), c(2, 3, 2, 2))))
Sr3 <- st_polygon(list(cbind(c(4, 4, 5, 10, 4), c(5, 3, 2, 5, 5))))
Sr4 <- st_polygon(list(cbind(c(5, 6, 6, 5, 5), c(4, 4, 3, 3, 4))))
sf_collection <- st_sf(geometry = st_sfc(Sr1, Sr2, Sr3, Sr4))
leaflet(height = "300px") %>% addPolygons(data = sf_collection)
