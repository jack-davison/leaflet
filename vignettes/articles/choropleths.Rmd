...
In this case, we'll use the `{sf}` package to load the data into `sf` objects, which will let us easily manipulate the geographic features, and their properties, in R.
...
Now, let's color the states according to their population density. You have [various options](https://rstudio.github.io/leaflet/articles/colors.html) for mapping data to colors; for this example we'll match the Leaflet.js tutorial by mapping a specific set of bins into RColorBrewer colors.
...
```{r results = 'hide'}
# From https://leafletjs.com/examples/choropleth/us-states.js
states <- sf::st_read("https://rstudio.github.io/leaflet/json/us-states.geojson")
