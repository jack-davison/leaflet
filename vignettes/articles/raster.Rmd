...
Two-dimensional `SpatRaster` and `SpatVector` objects (from the [`terra` package](https://CRAN.R-project.org/package=terra)) can be turned into images and added to Leaflet maps using `addRasterImage()`.

`addRasterImage()` works by projecting the `SpatRaster` or `SpatVector` object to [EPSG:3857](https://spatialreference.org/ref/epsg/3857/) and encoding each cell to an RGBA color, to produce a PNG image. That image is then embedded in the map widget.
